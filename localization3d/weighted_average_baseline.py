...
def estimate_sound_sources(args):
    ...
    (dataset_path / "bbox3dcenter_labels").mkdir(parents=True, exist_ok=True)
    (out_path / "sound_source_location3d").mkdir(parents=True, exist_ok=True)
    ...
    while webcam_rec.isOpened():
        ...
        if is_event_frame:
            ...
            # localization baseline: weighted average
            point_weights = point_scores / point_scores.sum()
            predicted_center_point = (point_weights[:, None] * xyz_trunc[pts_in_webcam]).sum(axis=0)
            localization_latencies.append(time.perf_counter() - start_time)
        else:
            predicted_center_point = None
        ...
        if is_event_frame:
            if gt_bbox is not None:
                gt_bbox_centers.append(np.asarray(gt_bbox.center))
            else:
                gt_bbox_centers.append(np.zeros((3)))
            if predicted_center_point is not None:
                pred_3d_locations.append(predicted_center_point)
            else:
                pred_3d_locations.append(np.zeros((3)))

        pbar.update()

    if len(gt_bbox_centers) > 0:
        gt_bbox_centers = np.stack(gt_bbox_centers)
    np.save(
        dataset_path / "bbox3dcenter_labels" / f"1_{args.scene_id:03d}_Movie2D_3dBboxCenters.npy",
        gt_bbox_centers,
    )
    if len(pred_3d_locations) > 0:
        pred_3d_locations = np.stack(pred_3d_locations)
    np.save(
        out_path / "sound_source_location3d" / f"1_{args.scene_id:03d}_Movie2D_3dLocation.npy",
        pred_3d_locations,
    )
    pbar.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out_path",
        default="data/prediction/event_localization3d_baseline/",
